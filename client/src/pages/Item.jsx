    ...
    <>
      <div className="flex flex-col lg:flex-row shadow-2xl p-6 sm:p-8 md:p-10 rounded-2xl m-6 sm:m-10 lg:m-20 mt-25 lg:mt-20 space-y-6 lg:space-x-6 lg:space-y-0">
        ...
        <div className="flex flex-col w-full sm:w-80 md:w-90 lg:w-100">
          <img
            className="border-2 border-black rounded-xl w-full h-full object-cover mb-4"
            src={item.imgURL || "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRGh5WFH8TOIfRKxUrIgJZoDCs1yvQ4hIcppw&s"}
            alt={item.name}
          />
        </div>
        <div className="flex flex-col w-full sm:w-80 md:w-90 lg:w-200">
          ...
          <div className="overflow-y-auto max-h-32 mb-6">
            <p className="text-base sm:text-lg text-black">{item.description}</p>
          </div>
          <p className="text-lg sm:text-xl text-green-600 font-bold mb-2">₱{formatPrice(item.price)}</p>
          ...
          <div className="mb-5 flex flex-col sm:flex-row items-center gap-4 sm:gap-10">
            ...
            <button
              ...
              className="bg-yellow-500 hover:bg-yellow-400 focus:ring-4 focus:ring-yellow-300 rounded-lg text-white font-bold py-2 px-4 w-full sm:w-1/2 mt-4 sm:mt-0 focus:outline-none dark:focus:ring-yellow-800"
            >
              Add to Cart
            </button>
          </div>
